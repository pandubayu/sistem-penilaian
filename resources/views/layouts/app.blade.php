...
<head>
    ...
    <style>
        ...
        /* ── Layout desktop: sidebar normal flow ── */
        ...
        /* ── Layout mobile: sidebar fixed overlay ── */
...
<body class="bg-gray-50 min-h-screen"
      ...
      @keydown.escape.window="sidebarOpen = false; document.getElementById('sidebar').classList.remove('open'); document.getElementById('sidebar-overlay').classList.remove('open');">
        ...
        <aside id="sidebar"
            ...
            <div class="px-4 py-5 border-b border-slate-700/60">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <button onclick="document.getElementById('sidebar').classList.remove('open'); document.getElementById('sidebar-overlay').classList.remove('open');"
                            class="ml-auto text-slate-400 hover:text-white p-1 rounded"
                            style="display:none"
            ...
            <nav class="flex-1 p-3 space-y-0.5 text-sm overflow-y-auto">
                ...
                @if(auth()->user()->isHr())
                    ...
                    <a href="{{ route('karyawan.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('karyawan.*') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        Karyawan
                    </a>
                    <a href="{{ route('bagian.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('bagian.*') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('periode.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('periode.*') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        Periode
                    </a>
                    <a href="{{ route('kriteria-teknis.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('kriteria-teknis.*') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('kriteria-umum.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('kriteria-umum.*') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                        Kriteria Umum
                    </a>
                    <a href="{{ route('grading.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('grading.*') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('mapping.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('mapping.*') ? 'active' : '' }}">
                        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01"/></svg>
                        Mapping Penilai
                    </a>
                    <a href="{{ route('laporan.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('laporan.*') ? 'active' : '' }}">
                    ...
                    <a href="{{ route('activity-log.index') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('activity-log.*') ? 'active' : '' }}">
                ...
                @if(auth()->user()->isPenilai())
                    ...
                    <a href="{{ route('penilaian.create') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('penilaian.*') ? 'active' : '' }}">
                ...
                @if(auth()->user()->isKaryawan())
                    ...
                    <a href="{{ route('hasil.saya') }}" onclick="closeSidebar()" class="sidebar-link {{ request()->routeIs('hasil.saya') ? 'active' : '' }}">
...
<div class="px-3 pb-1">
       ...
       class="sidebar-link {{ request()->routeIs('profil.*') ? 'active' : '' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0" id="main-wrapper">
            ...
            <header id="mobile-topbar"
                    class="sticky top-0 z-10 bg-slate-900 text-white px-4 py-3 flex items-center gap-3 shadow">
                <button onclick="document.getElementById('sidebar').classList.add('open'); document.getElementById('sidebar-overlay').classList.add('open'); document.getElementById('btn-close-sidebar').style.display='block';"
                        class="text-slate-300 hover:text-white p-1 rounded"
                ...
                <div class="ml-auto w-7 h-7 rounded-full bg-gradient-to-br from-green-500 to-indigo-600 flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                </div>
            </header>
    ...
    <script>
        // Fungsi tutup sidebar (dipanggil saat klik link menu)
